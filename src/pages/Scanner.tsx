...
export default function Scanner({ mode = 'staff' }: ScannerProps) {
  ...
  // Get recent orders from context (for staff mode)
  ...
  const printToThermalPrinter = useCallback((order: OrderDetails) => {
  ...
  const handleScan = useCallback(async (searchValue?: string) => {
    const searchTerm = searchValue || qrInput.trim();
    if (!searchTerm || scanning) return;
    ...
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { 
          ...
          width: { ideal: isKiosk ? 1280 : 640 },
          height: { ideal: isKiosk ? 720 : 480 }
        }
      });
  ...
  }, [scanQRFromCamera, isKiosk, toast]);

  // Auto-start camera on mount for kiosk mode
  useEffect(() => {
    if (isKiosk) {
      startCamera();
      
      // Request fullscreen for kiosk
